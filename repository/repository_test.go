...
func TestStoreDB_Create_InvalidInput(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	// Test with empty original URL
	err = store.Create("", "shortURL", "userID")
	assert.Error(t, err)

	// Test with empty short URL
	err = store.Create("originalURL", "", "userID")
	assert.Error(t, err)

	// Test with empty user ID
	err = store.Create("originalURL", "shortURL", "")
	assert.Error(t, err)

	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestStoreDB_Get_NoResults(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	mock.ExpectQuery("SELECT original_url, deletedFlag FROM urls WHERE short_id =").
		...
		WillReturnRows(sqlmock.NewRows([]string{"original_url", "deletedFlag"}))

	originalURL, err := store.Get("nonExistentShortURL", "")
	assert.Error(t, err)
	assert.Empty(t, originalURL)
	assert.Equal(t, "not found", err.Error()) // Assuming "not found" is the error message for no results
	assert.NoError(t, mock.ExpectationsWereMet())
}

func TestStoreDB_DeleteURLs_NoRowsAffected(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	mock.ExpectExec("UPDATE urls").
		WithArgs("nonExistentShortURL", "userID").
		WillReturnResult(sqlmock.NewResult(0, 0))

	err = store.DeleteURLs("userID", "nonExistentShortURL", updateChan)
	assert.Error(t, err)
	assert.Equal(t, "no rows affected", err.Error()) // Assuming "no rows affected" is the error message
	assert.Empty(t, updateChan)
	assert.NoError(t, mock.ExpectationsWereMet())
}
